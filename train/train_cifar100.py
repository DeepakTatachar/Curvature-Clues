...
def main():
    ...
    parser.add_argument('--parallel',               default=True,           type=str2bool,  help='Use data parallel')
    ...
    for epoch in range(start_epoch, num_epochs, 1):
        ...
        if global_rank == 0:
            ...
            if args.parallel:
                torch.save(net.module.state_dict(), args.model_save_dir + args.dataset.lower()+ '/' + model_name  + f'.ckpt')
            else:
                torch.save(net.state_dict(), args.model_save_dir + args.dataset.lower()+ '/' + model_name + f'.ckpt')
        
        if save_ckpt and global_rank == 0:
            logger.info("Saving checkpoint...")
            if args.parallel:
                torch.save(net.module.state_dict(), args.model_save_dir + args.dataset.lower()+ '/' + model_name  + '.ckpt')
            else:
                torch.save(net.state_dict(), args.model_save_dir + args.dataset.lower()+ '/' + model_name + '.ckpt')
    ...
    if args.val_split > 0.0:
        ...
        if global_rank == 0:
            if args.parallel:
                torch.save(net.module.state_dict(), args.model_save_dir + args.dataset.lower()+'/' + model_name  + '_final.ckpt')
            else:
                torch.save(net.state_dict(), args.model_save_dir + args.dataset.lower()+'/' + model_name + '_final.ckpt')
