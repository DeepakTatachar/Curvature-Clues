...
def main():
    ...
    parser.add_argument("--model_seed", default=0, type=int, help="Model seed")
    parser.add_argument("--eps", default=1.0, type=float, help="privacy eps")
    ...
    args.suffix = f"eps_{float(args.eps)}_seed_{args.model_seed}"
    ...
    handler = logging.FileHandler(os.path.join("./logs", f"score_dp_{args.dataset}_{args.arch}_{args.suffix}_zo_aug_curv_scorer_gpu9.log"))
    ...
    model_and_acc_path = os.path.join(args.model_save_dir, args.dataset.lower(), f"{args.dataset.lower()}_resnet18_eps_{args.eps}_seed_{args.model_seed}_lr_0.001_and_accountant.ckpt")
